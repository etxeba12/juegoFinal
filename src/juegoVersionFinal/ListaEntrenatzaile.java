...
public class ListaEntrenatzaile {
		...
		while(itrEntrenatzaileak.hasNext() && ondoGoaz) {
			...
			while( ondoGoaz && pokemonGeratuAurkariari) {
				...
				if(ondoGoaz) {
					...
					System.out.println("Aurkakoaren pokemon-a:"+unekoPoke.getIzena());
					unekoPoke.printBarra();
					System.out.print("[" + Math.floor(unekoPoke.getBizitza()) + "/" + unekoPoke.getMaxBizitza() + "]");
					System.out.println();
					System.out.println("óóóóóóóóóóóóóóóó");
					...
					System.out.println("Gure pokemon-a:"+gurePoke.getIzena());
					gurePoke.printBarra();
	...
	private void informazioaInprimatu() {
		Iterator<Entrenatzaile> itr = this.getIteradorea();
		while(itr.hasNext()) {
			Entrenatzaile unekoa = itr.next();
			unekoa.getNireListaPokemon().imprimatuPokemon();
		}
	}
